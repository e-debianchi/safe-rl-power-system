...
policy_kwargs = dict(net_arch=dict(pi=[32, 32], qf=[48, 32]))
...
cbf_agent = RCBF_SAC(policy="MlpPolicy", env=cbf_env, device='cpu', learning_rate=lr, learning_starts=learn_start, seed=seed,
                 policy_kwargs=policy_kwargs, kappa=50.0) # 2 maybe not the best, 3 similar, trying 10
...
sac_agent.save('saved_models/sac_power_system')

safe_agent.save('saved_models/safe_sac_power_system_0perc')
safe_agent.learn(total_timesteps=timesteps//10)
safe_agent.save('saved_models/safe_sac_power_system_10perc')
safe_agent.learn(total_timesteps=(timesteps//4-timesteps//10))
safe_agent.save('saved_models/safe_sac_power_system_25perc')
safe_agent.learn(total_timesteps=(timesteps-timesteps//4), progress_bar=True)
safe_agent.save('saved_models/safe_sac_power_system')

cbf_agent.learn(total_timesteps=learn_start+1)
cbf_agent.save('saved_models/rcbf_sac_power_system_0perc')
cbf_agent.learn(total_timesteps=(timesteps//10-learn_start-1))
cbf_agent.save('saved_models/rcbf_sac_power_system_10perc')
cbf_agent.learn(total_timesteps=(timesteps//4-timesteps//10))
cbf_agent.save('saved_models/rcbf_sac_power_system_25perc')
cbf_agent.learn(total_timesteps=(timesteps-timesteps//4), progress_bar=True)
cbf_agent.save('saved_models/rcbf_sac_power_system')